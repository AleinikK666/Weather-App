...
struct WeatherService {
    ...
    private let weatherURL = URL(string: "https://api.openweathermap.org/data/2.5/weather?appid=3407e56b90cdbcda588f7bb801a35709&units=metric")!

    func fetchWeather(cityName: String) {
        guard let urlEncodedCityName = cityName.addingPercentEncoding(withAllowedCharacters: .urlHostAllowed) else {
            delegate?.didFailWithError(self, .general(reason: "Некорректное название города"))
            return
        }
        let urlString = "\(weatherURL.absoluteString)&q=\(urlEncodedCityName)"
